...
Type* makeShortType(void) {
  Type* type = (Type*)malloc(sizeof(Type));
  type->typeClass = TP_SHORT;
  return type;
}

Type* makeLongType(void) {
  Type* type = (Type*)malloc(sizeof(Type));
...
ConstantValue* makeShortConstant(short s) {
  ConstantValue* value = (ConstantValue*)malloc(sizeof(ConstantValue));
  value->type = TP_SHORT;
  value->shortValue = s;
  return value;
...
ConstantValue* makeLongConstant(long l) {
  ConstantValue* value = (ConstantValue*)malloc(sizeof(ConstantValue));
  value->type = TP_LONG;
  value->longValue = l;
  return value;
...
ConstantValue* duplicateConstantValue(ConstantValue* v) {
  ...
  else if (v->type == TP_SHORT)
    value->shortValue = v->shortValue;
  else if (v->type == TP_LONG)
    value->longValue = v->longValue;
  else
...
void freeObject(Object* obj) {
  ...
  case OBJ_VARIABLE:
    ...
    // if (obj->varAttrs->type != NULL) free(obj->varAttrs->type);
    // printf("free variable2\n");
    // if (obj->varAttrs != NULL) free(obj->varAttrs);
    break;
  case OBJ_FUNCTION:
...
void initSymTab(void) {
  ...
  obj = createFunctionObject("READSTR");
  obj->funcAttrs->returnType = makeStringType();
  addObject(&(symtab->globalObjectList), obj);

  obj = createFunctionObject("READS");
  obj->funcAttrs->returnType = makeShortType();
  addObject(&(symtab->globalObjectList), obj);
  ...
  obj = createFunctionObject("READL");
  obj->funcAttrs->returnType = makeLongType();
  addObject(&(symtab->globalObjectList), obj);
  ...
  param->paramAttrs->type = makeShortType();
  ...
  obj = createProcedureObject("WRITEL");
  param = createParameterObject("l", PARAM_VALUE, obj);
  param->paramAttrs->type = makeLongType();
  addObject(&(obj->procAttrs->paramList), param);
  addObject(&(symtab->globalObjectList), obj);
  ...
  obj = createProcedureObject("WRITESTR");
  ...
  shortType = makeShortType();
  longType = makeLongType();
}
