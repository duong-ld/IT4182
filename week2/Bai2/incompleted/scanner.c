...
Token* readIdentKeyword() {
  ...
  if (ident == NULL) {
    error(ERR_CALLOCFAIL, lineNo, colNo);
  }
...
Token* readNumber() {
  ...
  int i = 0;
  Token* result = NULL;
  if (charCodes[previousChar] == CHAR_PERIOD) {
    strNum[0] = '0';
    strNum[1] = '.';
    checkDot = 1;
    i = 2;
    result = makeToken(TK_NUMBER, lineNo, colNo - 1);
  } else {
    result = makeToken(TK_NUMBER, lineNo, colNo);
  }
